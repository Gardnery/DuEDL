...
class BaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #         label = pseudo_label_generator_acdc(image, h5f["scribble"][:])
        #         sample = {'image': image, 'label': label, 'scribble': scribble}
        #     elif self.sup_type == "scribble":
        #         scribble = h5f[self.sup_type][:]
        #         label = h5f['label'][:]
        #         sample = {'image': image, 'label': label, 'scribble': scribble}
        #     sample = self.transform(sample)
        else:
...
class RandomGenerator_1(object):
    def __init__(self, output_size):
        self.output_size = output_size
    ...
    def __call__(self, sample):
        ...
        label = zoom(
            label, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        # scribble = zoom(
        #     scribble, (self.output_size[0] / x, self.output_size[1] / y), order=1)
        scribble = zoom(
            scribble, (256, 256), order=0)
        image = torch.from_numpy(
            image.astype(np.float32)).unsqueeze(0)
        label = torch.from_numpy(label.astype(np.uint8))
