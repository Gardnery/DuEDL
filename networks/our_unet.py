...
class UNet_CCT_our(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCT_our, self).__init__()
        ...
        for i in range(4):
        # 添加卷积层
            conv_layer = nn.Conv2d(params['class_num'], params['class_num'], kernel_size=3, stride=1, padding=1)
            self.L_convs.append(conv_layer)

            # 添加标准化层
            norm_layer = nn.BatchNorm2d(params['class_num'])
            self.L_convs.append(norm_layer)

            # 添加激活函数
            relu = nn.ReLU(inplace=True)
            self.L_convs.append(relu)
        # 全连接
        self.retina_L = nn.Conv2d(params['class_num'], params['class_num'], 3, padding=1)
    ...
    def forward(self, x):
        ...
        main_l = self.forward_single_L(main_seg)
        aux_l = self.forward_single_L(aux_seg1)
        return main_seg, aux_seg1,main_l,aux_l
