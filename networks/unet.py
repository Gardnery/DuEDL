...
class UNet_CCT1(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCT1, self).__init__()
        ...
        # 添加线性层分类头，用于Softmax
        self.Softmax_head = nn.Linear(params['feature_chns'][-1], class_num)

        # 添加MLP分类头，用于EDL
        self.EDL_head = MLP(params['feature_chns'][-1], [256, 128, 64, 32], class_num)
    ...
    def forward(self, x):
        ...
        # 线性层分类头
        main_linear_output = self.Softmax_head(main_seg)

        # MLP分类头
        main_mlp_output = self.EDL_head(main_seg)

        # 线性层分类头
        aux_linear_output = self.Softmax_head(aux_seg1)

        # MLP分类头
        aux_mlp_output = self.EDL_head(aux_seg1)


        return main_linear_output, main_mlp_output,aux_linear_output,aux_mlp_output
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim):
        ...
        layers.append(nn.ReLU())  # 添加第一个隐藏层的激活函数

        # 添加隐藏层到隐藏层的全连接层
        for i in range(len(hidden_dims) - 1):
            layers.append(nn.Linear(hidden_dims[i], hidden_dims[i + 1]))
            layers.append(nn.ReLU())  # 添加每个隐藏层的激活函数

        # 添加最后一个隐藏层到输出层的全连接层
        layers.append(nn.Linear(hidden_dims[-1], output_dim))

        # 将所有层打包成一个Sequential模块
