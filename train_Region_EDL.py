...
parser.add_argument('--exp', type=str,
                    default='ACDC/ACDC_pCE_double_Region', help='experiment_name')
...
def Region_loss(y, alpha, device=None):
    if not device:
        device = get_device()
    y = y.to(device)
    alpha = alpha.to(device)
    S = torch.sum(alpha, dim=1, keepdim=True) + 1e-8
    loglikelihood_var = alpha * (S - alpha) / (S * S * (S + 1))
    sDiceDen = y ** 2 + (alpha / S) ** 2

    loss = (y * (alpha / S)).sum(0) / ((sDiceDen + loglikelihood_var).sum(0))
    loss = 1 - 1 / 2 * loss.sum(-1)
    return loss
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            L_ace_main = Region_loss(label[:,:4],alpha_main, device="cuda") + annealing_coef * kl_divergence((alpha - 1) * (1 - label[:,:4]) + 1, 4, device="cuda")
            L_ace_aux = Region_loss(label[:,:4],alpha_aux, device="cuda") + annealing_coef * kl_divergence((alpha - 1) * (1 - label[:,:4]) + 1, 4, device="cuda")
